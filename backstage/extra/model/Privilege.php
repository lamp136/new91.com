...
class Privilege extends Model{
    ...
    public function getUsetPrivilege($userid){
        ...
        $priIdsData = array();
        $privileges = array();
        $roleUser = db('role_user')->where('user_id='.$userid)->select();
        if(!empty($roleUser)){
           ...
           $roleData = db('role')->where($map)->select();
           // var_dump($roleData);
           if(!empty($roleData)){
               foreach ($roleData as $key => $value) {
                   ...
                   //获取所有商务部门的ID
                    ...
                    // if(in_array($value['id'],config('BUSINESS_PRIVILEGE'))){
               ...
               if(!empty($priIdsData)){
                    ...
                    $privileges = db('Privilege')->where($where)->select();
                }
                if(!empty($privileges)){
