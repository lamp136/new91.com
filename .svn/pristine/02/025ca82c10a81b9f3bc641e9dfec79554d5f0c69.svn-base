...
class Cemetery extends Base
{
    ...
    public function index(){
        ...
        if(!empty($input['name'])){
            $search = new Search();
            $resultdata = $search->searchName($input['name'],config('category_cemetery_id'));
        ...
        $storeList = Db::name('store')->where($where)->field('id,name,store_sn,image,thumb_image,address,min_price,label,actual_hits,hits,status')->order('member_status DESC,sort DESC')->paginate(config('page_size'),false,['query' => $input]);
        ...
        if(!empty($selectCity)||strpos($seostring,'{region}')){
            $seo['seo_title'] = str_replace('{region}',$city[$selectCity],$seo['seo_title']);
            $seo['seo_keywords'] = str_replace('{region}',$city[$selectCity],$seo['seo_keywords']);
            $seo['seo_description'] = str_replace('{region}',$city[$selectCity],$seo['seo_description']);
        }
    ...
    public function details(){
        ...
        if($id){
            ...
            $tombZones = TombZone::withcount('tombs')->where('store_id',$id)->field('id,zone_name')->select();
            $allTombZones = array();
            foreach($tombZones as $val){
                $allTombZones[$val['id']] = $val['zone_name'];
            }
    ...
    public function celebrity(){
        ...
        if($id){
            ...
            $celebrity['content'] = str_replace('src="/public/','src="/',$celebrity['content']);
            if(!empty($celebrity)){
